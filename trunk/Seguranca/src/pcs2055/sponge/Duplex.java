...
public interface Duplex {
    ...
    */
    ...
    /**
    * Prepare for full-duplex operation at the desired security level.
    *
    * @param hashBits the desired hash size in bits 
    * (used only to specify the desired security level; 0 = default).
    */
    ...
    *
    * @param sigma data input (may be null; will be padded if shorter than the bit-rate).
    * @param sigmaLength its length in bytes (may be zero; must not exceed the bit-rate).
    * @param z data output (may be null).
    * @param zLength its length in bytes (may be zero; must not exceed the bit-rate).
    *
